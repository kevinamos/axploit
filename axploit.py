...
def arp_poison_callback(packet):
    ...
    if packet[ARP].op == 1 and str(
            packet[ARP].psrc) == str('192.168.100.5') and str(
            packet[ARP].pdst) == '192.168.100.1':
        ...
        txt1 = "Fooling " + packet[ARP].psrc + \
            " that " + packet[ARP].pdst + " is me"
        print txt1
        b.add_widget(Label(text=txt1, pos_hint={
                     'x': .1, 'top': .2}, size_hint=(.1, .1)))
    ...
    if packet[ARP].op == 1 and (router_ip):
        if(str(packet[ARP].psrc) == router_ip and str(packet[ARP].pdst) == str('192.168.100.5')):
            ...
            txt2 = "Now Fooling " + packet[ARP].psrc + \
                " that " + packet[ARP].pdst + " is me"
            print txt2
            b.add_widget(Label(text=txt2, pos_hint={
                         'x': .1, 'top': .2}, size_hint=(.1, .1)))
...
class SnifferScreen(Screen):
    ...
    def sniff_traffic(self, interface_choice):
        ...
        self.start_sniffer_object = threading.Thread(
            target=self.sniffer_object.start_sniffing, args=(
                interface_choice, ip_to_sniff)).start()
        if self.sniffer_object.error:
            warning = MenuScreen()
    ...
    def display_sniff_results(self, *args):
        ...
        try:
            if self.sniffer_object.matched_packet != '' and self.sniffer_object.source_ip != '' and self.sniffer_object.stop == '':
                ips = str(self.sniffer_object.destination_ip) + \
                    str(self.sniffer_object.source_ip)
                ...
                if int(len(self.sniffer_object.matched_packet)) > 300:
                    ...
                    if fh:
                        ...
                        self.sniffer_object.matched_packet = "packet too large to display. packet stored in " + \
                            dir_path + '/' + filename

                        fh.close()

                self.ip_list.append(ips)
                self.ids.sniffer_results.add_widget(
                    Label(
                        text=str(
                            self.sniffer_object.source_ip), color=(
                            0, 0, 0, 1), size_hint_y=None, height=100, pos_hint={
                            'top': 1}))
                self.ids.sniffer_results.add_widget(
                    Label(
                        text=str(
                            self.sniffer_object.destination_ip), color=(
                            0, 0, 0, 1), size_hint_y=None, height=100, pos_hint={
                            'top': 1}))
                self.ids.sniffer_results.add_widget(
                    Label(
                        text=str(
                            self.sniffer_object.matched_packet), color=(
                            0, 0, 0, 1), size_hint_y=None, height=100, pos_hint={
                            'top': 1}, text_size=(
                            265, None), haligh='left', valign='bottom'))
                self.packets_captured.append(
                    self.sniffer_object.matched_packet.strip())
                print "packet len is -----> " + \
                    str(len(self.sniffer_object.matched_packet))
        except :
            print"an error occured during sniffing"
...
class MenuScreen(Screen):
    ...
    def get_hosts(self):
        ...
        for i, host in enumerate(self.all_hosts):
            if i % 2 == 0:
                ...
                b = Button(text=str(host),
                           on_release=lambda x,
                           i=i: self.display_hosts(self.all_hosts[i + 1]),
                           text_size=(180,
                                      None),
                           halign='left',
                           color=(0,
        ...
        if self.ping_scan:
            ...
            self.ids.sr.add_widget(
                Label(
                    text=str(
                        host_info['ip']), color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}))
            # self.ids.sr.add_widget(Label(text=str('None'),color=(0,0,0,1),size_hint_y=None ,height=40, pos_hint={'top':1} ))
            self.ids.sr.add_widget(
                Label(
                    text=str(
                        host_info['vendor']), color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}))
            self.ids.sr.add_widget(
                Label(
                    text=str(
                        host_info['hostname']), color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}))
            self.ids.sr.add_widget(
                Label(
                    text=str(
                        host_info['state']), color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}))
            self.ids.sr.add_widget(
                Label(
                    text=str(
                        host_info['reason']), color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}))
        else:
            self.ids.sr.add_widget(
            ...
            for i, info in enumerate(host_info):
                if info:
                    ...
                    self.ids.sr.add_widget(
                        Label(
                            text=str(
                                host_info[i]['port']), color=(
                                0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                'top': 1}))
                    self.ids.sr.add_widget(
                        Label(
                            text=str(
                                host_info[i]['service_name']), color=(
                                0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                'top': 1}))
                    self.ids.sr.add_widget(
                        Label(
                            text=str(
                                host_info[i]['state']), color=(
                                0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                'top': 1}))
    ...
    def get_services(self):
        ...
        for i, service in enumerate(self.all_services):
            if service not in self.service_list:
                ...
                lb = Button(
                    on_release=lambda x, i=i: self.display_services(i), text=str(
                        service['service_name']), text_size=(
                        180, None), halign='left', color=(
                        0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                        'top': 1}, background_color=(
                        242, 242, 242, 1))
    ...
    def display_services(self, service_index):
        ...
        self.ids.sr.add_widget(
            Label(
                text=str(
                    self.all_services[service_index]['port']), size_hint_y=None, color=(
                    0, 0, 0, 1), height=30, pos_hint={
                    'top': 1}))
        self.ids.sr.add_widget(
            Label(
                text=str(
                    self.all_services[service_index]['service_name']), size_hint_y=None, color=(
                    0, 0, 0, 1), height=30, pos_hint={
                    'top': 1}))
        self.ids.sr.add_widget(
            Label(
                text=str(
                    self.all_services[service_index]['state']), size_hint_y=None, color=(
                    0, 0, 0, 1), pos_hint={
                    'top': 1}, height=30))
        self.ids.sr.add_widget(
            Label(
                text=str(
                    self.all_services[service_index]['host']), size_hint_y=None, color=(
                    0, 0, 0, 1), pos_hint={
                    'top': 1}, height=30))
                ...
                if not (correct_ip):
                    ...
                    if self.ids.scan_command.text == '':
                        if scan_type_choice == 'Syn Scan':
                            scan_type_choice = '-sS'
                        scan_return = scanner_object.nmapScan(
                            str(target_host), str(scan_type_choice))
                        if scan_return:
                            self.warning(scan_return)
                            return 0
                    else:
                        scan_return = scanner_object.nmapScan(
                            self.ids.scan_command.text)
                        ...
                        if '/' not in target_host and '-' not in target_host and '-sn' != scan_type_choice and '-sn' not in scan_arguments:
                            ...
                            if scanner_object.protocols:
                                try:
                                    ...
                                    for pr, proto in enumerate(
                                            scanner_object.protocols):
                                        ...
                                        for po, port in enumerate(lport):
                                            ...
                                            if scanner_object.version_detection:
                                                scanner_object.s_version.append(str(scanner_object.nmScan[scanner_object.tgtHost][proto][port]['product']) + str(
                                                    scanner_object.nmScan[scanner_object.tgtHost][proto][port]['version'] + " " + "(" + str(scanner_object.nmScan[scanner_object.tgtHost][proto][port]['extrainfo']) + ")"))
                                            ...
                                            self.all_services.append(
                                                self.service_dict)  # append the information of every service in a list
                                            scanner_object.service.append(
                                                scanner_object.nmScan[scanner_object.tgtHost][proto][port]['name'].strip())
                                            scanner_object.state.append(
                                                scanner_object.nmScan[scanner_object.tgtHost][proto][port]['state'])
                                            r"""self.ids.sr.add_widget(Label(text=str(scanner_object.service[po]) + "\ "+proto + "  \t "+str(scanner_object.state[po]) ,size_hint_x=None,width=200,size_hint_y=None, height=60, halign='left', pos_hint={'top':1}, color=(0,0,0,1))) """
                                            scanner_object.reason.append(
                                                scanner_object.nmScan[scanner_object.tgtHost][proto][port]['reason'].strip())
                                except BaseException:
                            ...
                            if 'v' in scan_type_choice or 'V' in scan_type_choice or scanner_object.version_detection:
                                self.ids.sr.cols = 5
                                self.ids.sr.add_widget(
                                    Label(
                                        text='VERSION',
                            ...
                            for i, pr in enumerate(lport):
                                ...
                                self.ids.sr.add_widget(
                                    Label(
                                        text=str(
                                            lport[i]), size_hint_y=None, color=(
                                            0, 0, 0, 1), height=30, pos_hint={
                                            'top': 1}))
                                self.ids.sr.add_widget(
                                    Label(
                                        text=scanner_object.service[i],
                                        size_hint_y=None,
                                        color=(
                                            0,
                                            0,
                                ...
                                if self.ids.sr.cols == 5:
                                    self.ids.sr.add_widget(
                                        Label(
                                            text=scanner_object.s_version[i],
                                            size_hint_y=None,
                                            color=(
                                                0,
                                                0,
                            ...
                            if scanner_object.os_detection:
                                ...
                                self.ids.sr.add_widget(
                                    Label(
                                        text=scanner_object.cpe, size_hint_y=None, color=(
                                            0, 0, 0, 1), text_size=(
                                            600, None), halign='left', pos_hint={
                                            'top': 1}, height=30))
                                self.ids.sr.add_widget(Label(text=''))
                                self.ids.sr.add_widget(Label(text=''))
                                self.ids.sr.add_widget(Label(text=''))
                                self.ids.sr.add_widget(
                                    Label(
                                        text=scanner_object.device_type, size_hint_y=None, color=(
                                            0, 0, 0, 1), text_size=(
                                            600, None), halign='left', pos_hint={
                                            'top': 1}, height=30))
                                self.ids.sr.add_widget(Label(text=''))
                                ...
                                self.ids.sr.add_widget(
                                    Label(
                                        text=scanner_object.accuracy, size_hint_y=None, color=(
                                            0, 0, 0, 1), text_size=(
                                            600, None), halign='left', pos_hint={
                                            'top': 1}, height=30))
                                self.ids.sr.add_widget(Label(text=''))
                            ...
                            self.ids.sr2.height = 600 * \
                                int(len(scanner_object.nmScan.all_hosts()))
                            file_scans = open('scans.txt', 'w')
                            # check to see if it is a ping scan that does not
                            # need port scanning
                            ...
                            if '-sn' in scan_arguments or scan_type_choice == '-sn':
                                ...
                                try:
                                    self.host_info_ping = []
                                    if int(
                                            scanner_object.nmScan.scanstats()['uphosts']) > 1:
                                        ...
                                        nm = scanner_object.nmScan
                                        host = scanner_object.nmScan.all_hosts()[
                                            0]
                                        ...
                                        self.ids.sr.add_widget(
                                            Label(
                                                text=str(
                                                    nm[host]['vendor']), color=(
                                                    0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                                    'top': 1}))
                                        self.ids.sr.add_widget(
                                            Label(
                                                text=str(
                                                    nm[host]['hostnames'][0]['name']), color=(
                                                    0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                                    'top': 1}))
                                        self.ids.sr.add_widget(
                                            Label(
                                                text=str(
                                                    nm[host]['status']['state']), color=(
                                                    0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                                    'top': 1}))
                                        self.ids.sr.add_widget(
                                            Label(
                                                text=str(
                                                    nm[host]['status']['reason']), color=(
                                                    0, 0, 0, 1), size_hint_y=None, height=40, pos_hint={
                                                    'top': 1}))
                            ...
                            for i, protocol in enumerate(
                                    scanner_object.nmScan[host].all_protocols()):
                                list_of_ports = scanner_object.nmScan[host][protocol].keys(
                                )
                                for port in list_of_ports:
                                    self.port_info = {}
                                    service = str(
                                        scanner_object.nmScan[host][protocol][port]['name'].strip())
                                    state = str(
                                        scanner_object.nmScan[host][protocol][port]['state'].strip())
...
class MitmScreen(Screen):
    ...
    def handle_dns_spoofing(self):
        ...
        if self.fake_ip == '':
            ...
            self.start_dnsspoof_thread = threading.Thread(
                target=self.spoofer_object.start, args=(
                    self.victim_ip, self.address_to_spoof, self.fake_ip,))
    ...
    def handle_arp_spoofing(self, *args):
        ...
        if self.arp_interface == '' or "Choose Interface" in self.arp_interface:
            ...
            self.arp_spoofing_thread = threading.Thread(
                target=self.arp_spoofing_object.start_arp_spoofing, args=(
                    self.arp_victim_ip, self.arp_interface, ))
    ...
    def stop_arp_spoofing():
        if self.arp_spoofing_thread == '':
            ...
            if self. arp_spoofing_object.stop:
...
class RseScreen(Screen):
    ...
    def server_listen(self, port_number):
        ...
        while True:
            if self.stop_rse_server:
                return 0
            self.client, addr = self.listener.accept()
            self.ids.serve.text = "Incomming connection from  %s: %d" % (
                addr[0], addr[1])
    ...
    def call_server(self, port_number):
        try:
            ...
            return 0
        self.start_server_thread = threading.Thread(
            target=self.server_listen, args=(port_number,))
        self.start_server_thread.start()
        self.ids.serve.text = "Server is listening on port " + \
            str(port_number) + "..."
...
class MainWindow(BoxLayout):
    ...
    def sniff_window(self):
        b = BoxLayout()
        b.add_widget(Label(text='Enter the Interface', pos_hint={
                     'x': .1, 'top': .2}, size_hint=(.1, .1)))
        b.add_widget(TextInput(multiline=False, pos_hint={
                     'x': .1, 'top': .2}, size_hint=(.1, .1)))
        return b

    def Mitm_window(self):
        self.b = BoxLayout()
        self.b.add_widget(Label(text='Enter the Interface', pos_hint={
                          'x': .1, 'top': .2}, size_hint=(.1, .1)))
        self.b.add_widget(TextInput(multiline=False, pos_hint={
                          'x': .1, 'top': .2}, size_hint=(.1, .1)))
        btn1 = Button(text='Start', pos_hint={
                      'x': .1, 'top': .2}, size_hint=(.1, .1))
    ...
    def arp_poison_callback(self, packet):
        ...
        if packet[ARP].op == 1 and (router_ip):
            if(str(packet[ARP].psrc) == router_ip and str(packet[ARP].pdst) == str('192.168.100.5')):
                ...
                device_b[ARP].pdst = packet[ARP].psrc
                txt2 = "Now Fooling " + \
                    packet[ARP].psrc + " that " + packet[ARP].pdst + " is me"
                print txt2
